...
class BaseRRLTrader():
    def __init__(
        ...
        Args:
            ...
            kind (str, optional): Reward Function. Defaults to "SR" (Sharpe Ratio).
        """
            ...
            for j in range(1, self.input_size+1, 1):
                if j > self.lookback : 
                    if self.feature is False:
                        ...
                        #self.input_vector[i][j+14] = self.Momentum_1M[i]
                        #self.input_vector[i][j+15] = self.Momentum_3M[i]
                        #self.input_vector[i][j+16] = self.Momentum_6M[i]
                        #self.input_vector[i][j+17] = self.Momentum_1Y[i]
                        self.feature = True
